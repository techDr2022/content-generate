...
/**
 * Password for both TechDr users after seeding (`contact@techdr.in`, `support@techdr.in`).
 * Set `TECHDR_PASSWORD` in the repo root `.env` (or `server/.env`), then run `npm run db:seed`.
 * Falls back to `techDr` if unset. Minimum 6 characters when set.
 */

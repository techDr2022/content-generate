...
import { z } from "zod";
import type { SpecialDay, TopicHistory } from "@prisma/client";
...
export function startGenerationWorker(): Worker<GenerateJobPayload> {
  const worker = new Worker<GenerateJobPayload>(
    ...
    async (job) => {
      ...
      const { jobId, clientId, month, year, userId, postCountOverride, extraSpecialDays } =
        job.data;
      const runStarted = Date.now();

      try {
        await job.updateProgress({ step: "started", pct: 1 });
        ...
        await job.updateProgress({ step: "prompt_sent", pct: 10 });
        await throwIfCancelled(jobId);
        ...
        await job.updateProgress({ step: "claude_responded", pct: 50 });
        await throwIfCancelled(jobId);
        ...
        await job.updateProgress({ step: "excel_building", pct: 75 });
        await throwIfCancelled(jobId);
        ...
        await job.updateProgress({ step: "upload_complete", pct: 100 });
        ...
        await job
          .updateProgress({ step: "failed", pct: 0, error: msg })
          .catch(() => undefined);
  ...
  worker.on("failed", async (job, err) => {
    if (!job?.data) return;
    const { jobId, userId, month, year } = job.data as GenerateJobPayload;
